# Plan: Phase 2 — continuous brackets, guarded tweens, explicit world namespacing
...
Two acceptance scripts already exist and should drive verification:
- `tests/stephen/new_api.script` — **already written**, exercises every Phase 2
  capability below. This is the primary target: after this phase, stepping
  through it should produce the behavior described in its narration. It
  intentionally uses **plain `[code]` (continuous) brackets as the default**
  for ordinary creation/mutation, and reserves `[!code]` for the two cases
  that are genuinely one-time actions (`audio_reactive` setup, `clear()`). Do
  not "fix" it by adding more bangs — see section B.
- `tests/stephen/main.script` — **already migrated** to the new syntax (no more
  `[world: code]` colon routing, no string-expression args). It has no
  `remove()`/`clear()` calls, so after migration it uses **zero** `[!code]`
  brackets — every bracket is plain/continuous. This is deliberate, not an
  oversight: reconciliation should make every one of its creation calls safe
  to re-run every tick. This script is also the main regression check for the
  `dynamicSize` guard in section D — its growing-circle line
  (`circle((c) => 50 * c.audioLevel + c.t, ...)`) is exactly the case that
  breaks if that guard is missing (see section D — read it before touching
  `frp.js`).
...
from files alone, so trust this document) converged on three changes:
...
- `listener/listen2/editor.html` owns: the master clock (`t`/`dt`), the
  microphone (`audioLevel`), the FRP engine (`frp.js`'s `dynamicProps`/
  `registerDynamic`/`applyDynamic`/`registerTween`/`applyTick`/etc. — loaded via
  `<script src="/interpreters/frp.js">`), and `colors.js`/`parseColor`. Also
  owns a per-world state store (`stateStores: {browser_2d: Map<id,{kind,props}>,
  browser_3d: Map<...>}`) and the verb functions (`circle`/`square`/.../`move`/
  `rotate`/.../`clear`), built by `makeWorld2DApi`/`makeWorld3DApi` and attached
  as **prototypes** of per-world namespace objects (`worldApis.browser_2d`,
  `worldApis.browser_3d`).
...
- `tick(nowMs)` (the master `requestAnimationFrame` loop) calls `applyTick(ctx)`
  (advances every registered dynamic across both worlds' state stores), then
  posts each world's *full current state* down as `applyOps`.
- Each world (`browser_2d.html`/`browser_3d.html`) is a generic per-`kind`
  ...
  them** — Phase 2 is entirely an editor-side (and `frp.js`-side) change.
...
- Remove `splitCommandTarget`, `DEFAULT_TARGET`, and `INTERPRETER_TARGETS`-as-a-
  ...
  doesn't change.
- A bracket's content is now just `code` (see form B below) — no colon-prefix
  parsing of any kind.
- `restart()` rebuilds **one** fresh `scriptScope` (re-populating
  `browser_2d`/`browser_3d` from the persistent `worldApis`, wiping user `let`-
  bindings), in addition to resetting the state stores (unchanged from Phase 1)
  and clearing the continuous-bracket registry (new — see section C).
- Minor known footgun, not worth guarding against: a script could shadow
  `browser_2d`/`browser_3d` via `let browser_2d = ...`. Same class of self-
  inflicted footgun as any global shadowing in JS; do nothing special about it.

### B. Two bracket forms — `[code]` is the default, `[!code]` is a narrow exception
...
- **`[code]`** (no bang) — **continuous, and this is the default form**. Use
  ...
  Reconciliation (section C) and the guarded tween (section D) exist
  ...
  Wrap it as `(c) => { code }`, evaluate **once** (against `scriptScope`) to
  produce a function value, then register that function — keyed by the
  bracket's stable source position (reuse the `{from, to}` offsets
  `computeWords()` already computes for the token) — to be invoked with `ctx`
  every tick, forever, from the moment it's triggered, until `restart()` or
  until the *same* bracket is re-triggered (which just replaces the
  registration — **no cleanup of whatever it previously created is required**;
  a re-landed continuous bracket may leave orphaned objects behind, same as
  Phase 1's one-shot `let c1 = circle(...)` already orphans the previous `c1`
  on re-trigger today — this is accepted, not a regression, and explicitly not
  being solved in this phase).
- **`[!code]`** — **one-off, an escape hatch, not the default.** Evaluate
  ...
  would mean section C or D has a bug, not that the script did something wrong.
...
### C. Creator reconciliation — needed only for the continuous form
...
- Every creator function (`circle`/`square`/`rect`/`ellipse`/`arrow`/`line`/
  ...
  - If active and the existing entry's `creatorFn` **matches** this call: do
    ...
    **Do not let this "recompute and write" step call `dynamicSize`'s
    `.register()` again for an already-dynamic size argument** — read section D
...
### D. Guards against re-registration corrupting in-flight behavior
...
a single tick, the value used to *capture* the starting point and the value
used to *sample* the result are the exact same `ctx` — so "start" and "now"
...
1. **`dynamicSize`'s growth ratio — fix this, it is not optional.**
   `dynamicSize`'s registered dynamic is `(ctx) => fn(ctx) / base`, where
   `base` is captured once, at the moment `.register()` runs. Both acceptance
   scripts rely on this via a growing-circle radius argument, e.g.
   `circle((c) => 50 + c.t, "red")`. Under Phase 2, that bracket is plain
   (continuous) by default (section B), which means its creator call is
   reconciled every tick (section C). If the reconciler's "update" path
   naively re-invokes `dynamicSize(...)` — and thus `.register()` — on every
   one of those reconciled calls, `base` gets recomputed as `fn(ctx_now)` on
   **every tick**, and that same tick's `tickDynamicProps()` samples the
   just-registered function with that **same** `ctx` — so
   `fn(ctx) / base` = `fn(ctx) / fn(ctx)` = **1**, every single tick. The
   circle would freeze at whatever size it happened to be instead of growing,
   silently, with no error. This is not a corner case — it is exactly what
   both acceptance scripts do, and it is exactly the kind of bug that looks
   fine in code review and only shows up when you actually watch it run (or
   trace it by hand, as this plan does above).
...
### E. What does *not* need a guard — verify this claim, don't just assume it

`dynamicColor`/`dynamicProp`'s registered ticking functions are
`(ctx) => transform(arg(ctx))` — they reference no externally-captured
...
confirm this by tracing it the same way section D's bug was traced, don't take
...
tracing through section D's bug is to know precisely which mechanism has the
hazard (one that reads a captured value inside its own ticking formula) and
which doesn't.

## File-by-file task list

1. **`interpreters/browser/frp.js`** — two changes, both from section D:
   - Add the existence-guard inside `dynamicSize` so it's safe to call every
     tick without re-registering an already-active dynamic. **Do this one
     first and verify it against `main.script`'s growing-circle line before
     moving on** — it's the higher-risk, higher-consequence of the two.
   - Add the guarded-tween logic to `registerTween`/`applyDynamic`'s
     numeric+duration branch, keyed by `(item, path)`.
...
2. **`listener/listen2/editor.html`**:
   - Replace the per-world `worlds`/`freshWorldScope`/`resetWorldState`'s
     ...
     (unchanged from Phase 1). `resolveHandle`, the state stores, and the
     verb-function implementations themselves (`makeWorld2DApi`/
     `makeWorld3DApi`) do **not** need to change — only how their namespaces
     are exposed to script code.
   ...
   - Replace `sendCommand`/`evalInWorld` with the two-form dispatch from
     section B: parse leading `!`, either one-shot-eval against `scriptScope`
     (reusing `globalizeDeclarationsForWith`, retargeted at `scriptScope`), or
     wrap-and-register for continuous invocation.
   - Add the continuous-bracket registry (a `Map` from bracket id to its
     registered function) and invoke every entry once per tick, inside the
     ambient-reconciliation-context push/pop from section C, before/after
     posting `applyOps` (order relative to `tickDynamicProps`/`applyTick`:
     continuous brackets should run *before* `applyTick`, since they're what
     call creators/mutators that register or update dynamics — `applyTick`
     then samples the current state of everything, including whatever this
     tick's continuous-bracket calls just set up).
   - Add the ambient reconciliation context (module-level variable) and the
     per-bracket call-site registry from section C, threaded into
     ...
     "allocate id" per creator, as described in section C).
   - Update `restart()` to also clear the continuous-bracket registry.
   - Update `RESET()` handling per section B.
...
- [ ] **The most important check, do this first**: with `main.script`'s
      `circle((c) => 50 * c.audioLevel + c.t, ...)` bracket (plain, continuous
      — no bang), confirm the circle's radius actually **keeps growing** over
      several seconds, sampled at multiple points in time, not just "it
      appears once and looks fine." If it renders once and then stays a fixed
      size, the `dynamicSize` guard (section D) is missing or wrong — this is
      the exact failure mode it silently produces. Don't rely on eyeballing a
      single frame.
- [ ] Walk through `new_api.script` end to end and confirm each narrated
      behavior, noting these are now **plain/continuous by default**, not
      one-off: the cube persisting via reconciliation without duplicating
      (`[let block = browser_3d.cube(...)]`, no bang), rotation driven by
      `(c) => c.t * 15`-style functions (no string expressions anywhere), the
      growing circle (same guard as above), the circle→square swap at `t=10`
      with **no duplicate object** left behind (verify by checking the state
      store only ever holds one entry for that slot, not by eyeballing
      rendering alone), the position flip-flop **not restarting every frame**
      (only re-easing every 5 seconds), the mixed-world bracket (one bracket
      creating a 2D circle and pulsing a 3D mesh's opacity), and — the only
      two `[!...]` brackets in either script — audio-reactivity setup and the
      final `clear()`, both one-off by deliberate choice (see section B for
...
- [ ] Grep for dead references to the removed per-world `with`-scoping
      ...
      `DEFAULT_TARGET`, `INTERPRETER_TARGETS` used for routing).
