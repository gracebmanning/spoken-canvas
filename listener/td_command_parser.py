...
====================
Parses [td: ...] command strings from .script files into structured JSON
messages ready to be sent over WebSocket to the TouchDesigner interpreter.
 
Supports two forms:
    Assignment:  td: let c1 = circle(50, "red")
    Bare call:   td: circle(50, "red")
...
def parse_td_command(raw):
    """
    Parse a raw td: command string into a structured dict.

    Args:
        raw: Full bracket contents from the .script file,
             e.g. "td: let c1 = circle(50, 'red')"

    Returns:
        Structured dict ready to be JSON-serialized and sent over WebSocket,
        or None if the string does not start with "td:"

    Raises:
        ValueError: If the command is recognized as a td: command but is malformed
                    or uses an unsupported function name
    """
    raw = raw.strip()

    if not raw.startswith('td:'):
        return None
    ...
    if match:
        ...
        match = CALL_PATTERN.match(body)
        if match:
            var_name = None
            func_name = match.group(1)
            args_str = match.group(2)
        else:
            raise ValueError(f"Could not parse td: command body: '{body}'")
...
if __name__ == '__main__':
    tests = [
        # (input, should_raise)
        ('td: let c1 = circle(50, "red")',       False),
        ("td: let myCircle = circle(100, 'blue')", False),
        ('td: let bg = circle(960, "#FFA500")',   False),
        ('td: circle(50, "red")',                 False),  # bare call, no name
        ('td: let c1 = circle(50)',               True),   # missing color arg
        ('td: let c1 = rotate(50, "red")',        True),   # unsupported command
        ('browser: let c1 = circle(50, "red")',   False),  # not a td: command → None
    ]
